...
async function initPage(tabName) {
	...
	if (!byId(tabName + "Page")) {
		...
		if (tabName == "welcome") {
			...
			byId("sample-unread-count").style["background-color"] = await storage.get("unreadCountBackgroundColor");
            ...
            addEventListeners("#unread-count-background-color", "color-changed", async e => {
                ...
                byId("sample-unread-count").style["background-color"] = color;
                sendMessageToBG("updateBadge");
            });
            ...
            onClick("#fetchOtherContacts", async () => {
                await niceAlert("Click OK to restart the extension");
                reloadExtension();
            });
...
function addPaperItem(params) { // node, value, label, prepend
	...
	if (params.icon) {
		...
		emptyNode(paperItem); // patch seems polymer would add shadydom when creating the paper-icon-item so i had to remove it
		paperItem.append($ironIcon, params.label);
	} else {
...
async function loadAccountsOptions(loadAccountsParams = {}) {
                ...
                function handleDrop(e) {
                    ...
                    if (dragSrcEl !== this) {
                        if (accountAddingMethod == "autoDetect") {
                            niceAlert("Option 1: Change to the Add Account option above then drag the accounts to reorder them.<br>Option 2: Sign out and back into the accounts in the order you want to see them in the extension.");
                            highlightAddAccount();
                        } else {
                            // swap
    ...
    if (!$accountsList._delegatesAttached) {
        ...
        chrome.permissions.contains({origins: [Origins.GMAIL]}, async result => {
            if (!result) {
                ...
                if (response == "ok") {
                    ...
                    if (granted) {
                        niceAlert(`${getMessage("emailAccounts")}: ${accounts.length}<br><br>You can customize their settings in the Accounts tab.`);
                    }
                }
            }
        });
