    ...
    chrome.alarms.onAlarm.addListener(async alarm => {
        ...
        if (alarm.name == Alarms.EVERY_MINUTE) {
            ...
            if (await storage.get("unreadCount") >= 1) {
                ...
                if (repeatNotification && await Math.abs((await storage.get("_lastRepeatNotificationTime")).diffInMinutes()) >= repeatNotification) {
...
chrome.browserAction.onClicked.addListener(async tab => {
	...
	if (browserButtonAction == BROWSER_BUTTON_ACTION_CHECKER_PLUS_POPOUT || browserButtonAction == BROWSER_BUTTON_ACTION_GMAIL_INBOX_POPOUT) {
		...
		if (accountToOpen) {
			...
			} else if (browserButtonAction == BROWSER_BUTTON_ACTION_GMAIL_DETACHED) {
...
async function showNotification(params = {}) {
    ...
    if (desktopNotification) {
        const dndState = await getDNDState({notificationRelated: true});
        if (dndState) {
...
async function createNotification(params) { // expected args: options, buttonsWithValues, emails
    ...
    if (DetectClient.isFirefox()) {
        ...
        if (!DetectClient.isMac() && (!await storage.get("moveIntoActionCenter") && await storage.get("notificationWakesUpScreenTemporarily"))) {
...
async function openInPopup(params = {}) {
	...
	chrome.windows.create(createWindowParams, async newWindow => {
...
if (chrome.idle.onStateChanged) {
    chrome.idle.onStateChanged.addListener(async newState => {
        ...
        if (newState == "active") {
            ...
            if (await storage.get("unreadCount") >= 1 && (Math.abs(lastNotificationDate.diffInSeconds()) > MIN_SECONDS_BETWEEN_NOTIFICATIONS || detectSleepMode.isWakingFromSleepMode()) && await storage.get("_lastNotificationDateWhileActive") < lastNotificationDate) {
		...
		if (unsnoozedEmails.length || !lastNotificationAccountDates[accountWithNewestMail.id] || passedDateCheck) {
			...
			if (unsnoozedEmails.length || newestMailObj[accountWithNewestMail.getEmail()] != mostRecentNewEmail.id) {
				...
				if (params.source != Source.STARTUP || (params.source == Source.STARTUP && await storage.get("showNotificationsOnStartup"))) {
					getDNDState({animationRelated: true}).then(function(dndState) {
						if (!dndState) {
							buttonIcon.startAnimation();
						}
					});
...
function playNotificationSound(source) {
	return new Promise(async (resolve, reject) => {
		...
		getDNDState({soundRelated: true}).then(async dndState => {
			if (dndState || source == "") {
				resolve();
			} else {
