...
function initOpenEmailEventListeners() {
	...
	onClick("#revertAutoSizing", function() {
		byId("openEmail").toggleClass("resized");
	});
		        ...
		        switch( match.getType() ) {
		            case 'url' :
		                ...
		                if( match.getUrl().indexOf( 'mysite.com' ) === -1 ) {
		                    ...
		                    //tag.classList.add( 'external-link' );
		                    return tag;
		                } else {
...
function openEmailPromise(params) {
	return new Promise(function(resolve, reject) {
        ...
        if (!$openEmailSection._modifiedScrollbarCss) {
            if (globalThis.scrollbarCssSheet) {
                const shadowRoot = document.querySelector("#openEmailSection app-header-layout")?.shadowRoot;
                if (shadowRoot) {
            ...
            $openEmailSection._modifiedScrollbarCss = true;
        }
			...
			if (mail.messages.last()) {
				...
				mail.messages.forEach(function(message, messageIndex) {
					...
					if (alwaysDisplayExternalContent) {
						...
						var filteredHTML = $messageBody.innerHTML;
						...
						if (!mail.forceDisplayImages) {
							$messageBody.querySelectorAll("img[src], imghidden[src], input[src]").forEach(el => {
				...
				function initReply() {
                    ...
                    emptyNode($sendAndArchive);
                    $sendAndArchive.append(`${getMessage("send")} + `, $archiveIcon);
                    ...
                    emptyNode($sendAndDelete);
                    $sendAndDelete.append(`${getMessage("send")} + `, $deleteIcon);
                ...
                function sendButton(event) {
                    ...
                    const previousSendButtonHTML = $sendButtonClicked.innerHTML;
					$sendButtonClicked.innerHTML = "<paper-spinner class='white' active></paper-spinner>";
                    ...
                    showToast({toastId:"undoToast", duration:999, text: `${getMessage("sending")}...`, actionParams:{
                            onClick: function() {
                                ...
                                $sendButtonClicked.innerHTML = previousSendButtonHTML;
                            }
                        }
                    });
...
function openComposeSection(params) {
	...
	*/
    ...
    onClickReplace("#sendComposeEmail", function(event) {
        ...
        event.target.innerHTML = "<paper-spinner class='white' active></paper-spinner>";
...
function initInboxMailActionButtons($mail) {
	if ($mail) {
		...
		if ($inboxMailActionButtonsTemplate) {
            ...
            initButton(".markAsUnread", "unreadLinkTitle", "markAsUnread", "stay", () => {
                $mail.classList.add("unread");
                updateUnreadCount(+1, $mail);
            });

			initButton(".reply", "reply", "reply", "stay", () => {
                setTimeout(() => {
                    closeWindow();
                }, 100);
            });
...
async function init() {
	...
	if (previewMailId && (await storage.get("browserButtonAction") != BROWSER_BUTTON_ACTION_GMAIL_INBOX && await storage.get("browserButtonAction") != BROWSER_BUTTON_ACTION_GMAIL_INBOX_POPOUT)) {
		...
		polymerPromise2.then(function() {
			setTimeout(function() {
				...
				const accountEmail = getUrlValue(location.href, "accountEmail", true);
    ...
    onClick(document.body, function() {
        clearCloseWindowTimeout(true);
