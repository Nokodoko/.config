...
function createPanelIframe() {
  ...
  panelIframe.className = 'lever-hire-frame';
  document.body.appendChild(panelIframe);
  new CrossDomainDragAndDrop(panelIframe);
}
...
function createToastIframe(action) {
  ...
  toastIframe.className = `lever-hire-toast-frame ${action}`
...
// Add a keyboard shortcut that opens the Lever Extension
