...
function openMobilePayDialog(params) {
    ...
    $dialog.querySelector(".dialogDescription").innerHTML = `
        ${params.message}
        <br><br>
...
async function initPaymentProcessor(price) {
	if (donateButtonClicked == "paypal") {
        ...
        //addFormField(form, "cloudVersion", 	"stripechanges"); // test
...
function signOut() {
	location.href = "https://accounts.google.com/Logout?service=cl&continue=https://calendar.google.com";
}
    ...
    */
    ...
    if (canHaveALicense(email)) {
        ...
        */
		...
		const action = getUrlValue(location.href, "action");
        ...
        if (action == "paypalSuccess" || action == "stripeSuccess") {
			...
			const error = getUrlValue(location.href, "error") || "";
