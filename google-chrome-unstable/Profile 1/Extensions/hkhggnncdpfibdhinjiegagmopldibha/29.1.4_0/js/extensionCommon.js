...
if (location.href.includes("popup.html")) {
	inPopup = true;
} else if (location.href.includes("reminders.html")) {
	inReminders = true;
}
...
if (inPopup) {
	...
	polymerFile = "vulcanized-polymer-reminders.js";
} else {
...
var pageVisible = new Promise((resolve, reject) => {
	if (document.hidden) {
        ...
        let delay;
        if (inReminders) {
            if (DetectClient.isMac()) { // delay polymer loading becase was taking too long on Mac
                delay = 1000;
            } else {
                delay = 500;
            }
...
var polymerPromise2 = (async function() {
    ...
    await pageVisible;
    await sleep(DetectClient.isMac() ? 1000 : 500);
...
function openDialog(idOrObject, params = {}) {
	return new Promise(async function(resolve, reject) {
        ...
        await sleep(1); // changed from 100
...
function openGenericDialog(params) {
	return new Promise((resolve, reject) => {
		...
		polymerPromise2.then(() => {
            ...
            params.title ||= "";
			$dialog.querySelector(TITLE_SELECTOR).innerHTML = params.title;
		
            params.content ||= "";
			
            if (typeof params.content == 'string') {
                $dialog.querySelector(CONTENT_SELECTOR).innerHTML = params.content;
			} else {
                emptyNode($dialog.querySelector(CONTENT_SELECTOR));
                $dialog.querySelector(CONTENT_SELECTOR).append(params.content);
			}
