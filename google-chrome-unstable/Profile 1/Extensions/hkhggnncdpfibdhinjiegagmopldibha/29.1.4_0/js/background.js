...
async function closeNotifications(notifications, params = {}) { // lastAction, skipNotificationClear
	...
	notificationsCloned.forEach(notification => {
        ...
        if (notification.event.kind == TASKS_KIND && params.lastAction != "snooze") {
            setTaskStatus(notification.event, TaskStatus.COMPLETED);
        }
	});
	...
	return new Promise(async (resolve, reject) => {
		...
		if (await isDND() || source == null || source == "") {
            ...
            function audioStopped(event) {
				...
				if (!(changedSrc && event.type == "abort") && !audioEventTriggered) {
            ...
            notificationAudio.addEventListener("ended", audioStopped, {signal: globalThis.controller.signal });
            notificationAudio.addEventListener("error", audioStopped, {signal: globalThis.controller.signal });
            notificationAudio.addEventListener("abort", audioStopped, {signal: globalThis.controller.signal });
...
async function playNotificationSound(notification, params = {}) {
	...
	if (!await isDND()) {
		...
		if (notificationSound) {
			playNotificationSoundFile(notificationSound, false).then(() => {
				playVoiceNotification(textToSpeak, params);
			});
		} else {
			playVoiceNotification(textToSpeak, params);
		}
        ...
        await asyncForEach(sendResponse.items, async item => {
            if (item.status == "cancelled" && !item.recurrence) {
                ...
                if (index == -1) { // add
                    ...
                    function modifyEventInArray(notificationsOpened, item) {
                        return notificationsOpened.some(notification => {
                            if (notification.event.id == item.id) {
                                notification.event = item;
                                initEventObj(notification.event, params.calendarId); // v2 had to initiate calendarId with 2nd parameter, v1 don't know if it's necessary but trying to figure this issue: https://jasonsavard.com/forum/discussion/6979/gettime-error-in-checkerplus-for-google-calendar#latest
                                return true;
                            }
                        });
                    }
                    ...
                    if (globalThis.notificationsOpened) {
                        modifyEventInArray(globalThis.notificationsOpened, item);
                    }

                    const notificationsOpened = await storage.get("notificationsOpened");
                    if (notificationsOpened) {
                        const found = modifyEventInArray(notificationsOpened, item);
...
async function pollServer(params = {}) {
    ...
    if (await storage.get("_firstLoad")) {
        chrome.browserAction.setBadgeBackgroundColor({color:BadgeColor.EMOJI});
    ...
    if (!await storage.get("loggedOut")) {
        ...
        events.forEach(event => {
            ...
            if ((showBusyEvents || calendar.accessRole != CalendarAccessRole.FREE_BUSY)
                ...
                && !hasUserDeclinedEvent(event, email)
                ...
                if (passedExcludeRecurringEventsButtonIcon && passedHiddenCalendarsFromButtonTest && !event.allDay && event.startTime.isBefore() && event.endTime?.isAfter()) {
                ...
                if (currentEventInProgress || (passedExcludeRecurringEventsButtonIcon && passedHiddenCalendarsFromButtonTest && (event.startTime.getTime() - Date.now() >= 0 || event.allDay && isToday(event.startTime)) && nextEventMin < 60*24*maxDaysAhead)) {
                    if (!nextEvent) {
                        if (event.allDay) {
        ...
        const soundsPlayedPromise = asyncForEach(notificationsQueue, async notification => {
            if (!notification.audioPlayedCount) {
                await playNotificationSound(notification, params);
            }
        });
...
chrome.runtime.onInstalled.addListener(async details => {
    ...
    if (details.reason == "install" && !await storage.get("installDate")) {
        ...
        // renamed option, sept 9, 2021
        if (await storage.get("pastDays")) {
            await storage.enable("dimPastEvents");
            await storage.remove("pastDays");
        }
...
if (chrome.commands && DetectClient.isChromium()) {
